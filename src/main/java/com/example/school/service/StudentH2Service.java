...
public class StudentH2Service implements StudentRepository {
    ...
    public String addMultipleStudents(ArrayList<Student> studentsList) {
        for (Student eachStudent : studentsList) {
            ...
            db.update("insert into student(studentName,gender,standard) value (?,?,?)", eachStudent.getStudentName(),
                    eachStudent.getGender(), eachStudent.getStandard());
        }
        String responseMessage = String.format("Successfull added %d students", studentsList.size());
